...
// Capture-time validating constructor: safeParse `{ at }` and translate a zod failure into the
// kernel's ValidationError (bad caller INPUT, like makeNode's ZodError→ValidationError wrap).
// capture() uses this — never a raw CapturedSchema.parse, which would leak a ZodError.
export function makeCaptured(at: string): Captured;          // ValidationError on malformed `at`

export function capturedOf(node: Node): Captured;            // FacetError if missing/malformed (read boundary)
...
// Normalized, offset-independent sort key over the stored WALL CLOCK. Validates `at`, strips the
// offset, and canonicalizes the time to a fixed width (seconds default to "00", fractions padded
// to a fixed precision) so plain string compare matches true wall-time order. This is required:
// raw lexicographic compare on `at` misorders e.g. "…09:00:00.500Z" before "…09:00:00Z".
export function capturedSortKey(at: string): string;        // e.g. "2026-06-24T09:00:00.000"
...
- `makeCaptured` (capture-time, caller input) throws `ValidationError`; `capturedOf` (read of a
  stored facet) throws `FacetError`. Both translate zod failures so a raw `ZodError` never escapes —
  `runCli` catches `ValidationError`/`FacetError` (both `NodesError`) as exit 1, never reaching its
  `throw e` fall-through.
...
- `capturedDate`/`capturedTime`/`capturedSortKey` are the **single** datetime contract for the rest
  of the app. Each parses `{ at }` through `CapturedSchema` before extracting, so no second implicit
  datetime semantics can grow in CLI or journal code. They return the **stored wall-clock** parts
  (the date/time as written, regardless of offset form) — never a runtime-timezone re-interpretation.
...
// (the CLI safeParses it and translates a failure to CliUsageError; journalView translates to
// ValidationError — same schema, boundary-specific typed error, no raw ZodError ever leaks).
...
`journalView`:
1. Validates `window.since` and `window.until` by **safeParsing** `JournalDateSchema` and throwing
   `ValidationError` on failure (never a raw `parse`/`ZodError`); rejects `since > until`
...
3. For each remaining thought, computes `capturedDate(capturedOf(node).at)`; keeps those with
   `since <= date <= until`.
4. Sorts entries in **stored-local-timestamp order** via `capturedSortKey(at)` — the normalized,
   offset-independent wall-clock key (so `…09:00:00.500Z` correctly follows `…09:00:00Z`, and two
   same-date thoughts under different offsets order by their stored wall clock, explicitly **not**
   UTC instant), tie-broken by `id`.
...
- `capture` attaches `facets[CAPTURED] = makeCaptured(at)` alongside `visualIdentity`, and sets
  `metadata.created = capturedDate(at)` as a **derived coarse projection** — never a competing source
  of truth. `metadata.updated` is left untouched this slice. (`makeCaptured`, not a raw
  `CapturedSchema.parse`, so a malformed `at` raises `ValidationError`, not an uncaught `ZodError`.)
...
Add from `./captured.js`: `CAPTURED`, `CapturedSchema`, `type Captured`, `makeCaptured`,
`capturedOf`, `requireValidCaptured`, `capturedDate`, `capturedTime`, `capturedSortKey`, `localIso`.
Add from `./journal.js`: `journalView`, `JournalDateSchema`, `type JournalEntry`, `type DayGroup`,
`type DateWindow`.
...
- Each date arg is **safeParsed** through `JournalDateSchema`; a failure is translated to
  `CliUsageError(JOURNAL_USAGE)` (exit 2) — the CLI never lets a `ZodError` escape.
...
**No raw `ZodError` leaks.** Every zod validation in SP7 is performed through a wrapper that
translates failure into a typed error the boundary already handles; nothing calls a bare
`schema.parse` on a value that could reach `runCli`. `runCli`'s catch chain (`CliUsageError`→2;
`NodesError`/`CliError`/`ConfigError`→1) is otherwise unchanged, and its `throw e` fall-through is
never reached by a validation failure.

- **Malformed `at` at capture** → `ValidationError` via `makeCaptured`, before any write → nothing
  persists (same atomic path as identity/tags). Caught as `NodesError` → exit 1.
- **Missing/malformed `captured` on read** → `FacetError` via `capturedOf`/`requireValidCaptured`
  during validation (exactly like `visualIdentity`). Caught as `NodesError` → exit 1.
- **Bad `journal` arguments** (malformed date, lone `--since`/`--until`, duplicate flag, positional +
  flags, `since > until`) → `CliUsageError` → exit 2 with `JOURNAL_USAGE`. Date args reach this via a
  `JournalDateSchema.safeParse` → `CliUsageError` translation.
- **`journalView` self-protection** — malformed bound or `since > until` → `ValidationError` (via
  safeParse, defense in depth for any non-CLI caller).
...
- `makeCaptured(bad)` throws a **`ValidationError`** (kernel `NodesError`), **not** a raw `ZodError`;
  `makeCaptured(good)` returns `{ at }`.
- `capturedDate` returns the stored date for both offset forms; `capturedTime` returns `HH:MM`,
  ignoring seconds/fractions.
- `capturedSortKey` is offset-independent and fraction-normalized: `…09:00:00.500Z` yields a key
  **greater** than `…09:00:00Z` (the raw-lexicographic bug), and `…09:00:00-04:00` /
  `…09:00:00+02:00` (same stored wall clock) yield **equal** keys.
...
- **Fractional ordering:** a `…:00.500Z` thought sorts **after** a `…:00Z` thought of the same date
  (guards against raw-lexicographic regression).
- **Wall-time semantics:** two same-date thoughts whose `at` carry different offsets sort by the
  stored wall clock (via `capturedSortKey`), **not** by absolute UTC instant (explicit assertion).
- Non-`THOUGHT` nodes passed in are filtered out (no throw).
- Empty days omitted from the result.
- Malformed bound → throws **`ValidationError`** (not `ZodError`); `since > until` → throws `ValidationError`.
...
4. **`at` shape** — `z.string().datetime({ offset: true })`: explicit offset required (`Z` allowed),
   naive/date-only rejected, fractional seconds allowed. Because fractions/offset forms vary, ordering
   never compares the raw string (see #9).
...
8. **Centralized datetime contract** — `capturedDate`/`capturedTime`/`capturedSortKey` validate
   through `CapturedSchema` first; CLI/journal never slice a raw string, so no second timestamp
   semantics grows.
9. **Stored-local sort via a normalized key** — `journalView` orders by `capturedSortKey(at)`, an
   offset-independent, fraction-normalized wall-clock key (**not** the raw `at` string, which
   misorders fractional seconds, and **not** UTC instant); tie-break by `id`.
14. **No raw `ZodError` leaks** — every zod check is wrapped to a typed error per boundary:
    `makeCaptured` → `ValidationError` (capture input), `capturedOf` → `FacetError` (stored read),
    `journalView` bounds → `ValidationError`, CLI date args → `CliUsageError`. `runCli`'s catch chain
    is unchanged and its `throw e` fall-through is never reached by a validation failure.
