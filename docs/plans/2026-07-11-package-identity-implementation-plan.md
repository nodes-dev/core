...
- Produces: distribution metadata named `nodes-core` while preserving the import package `nodes`.
...
- [ ] **Step 2: Change only the PEP 621 distribution name**
...
- [ ] **Step 3: Verify the new distribution/import boundary**

Run from the repository root:

```bash
rg -n '^name = "nodes-core"$|packages = \["src/nodes"\]' python/pyproject.toml
```

Expected: exactly two matches: `name = "nodes-core"` and `packages = ["src/nodes"]`.
...
- [ ] **Step 5: Commit the Python distribution rename**
...
- Produces: npm metadata named `@nodes-dev/core`, with identical version, exports, files, scripts, dependencies, and runtime API.
...
- [ ] **Step 2: Update the npm manifest name**

Change only the top-level name in `ts/package.json`:
...
Expected: `npm pkg get name` prints `"@nodes-dev/core"`; `rg` reports three matches; the diff changes only the three package-name values.
...
- Produces: evidence that the mechanical slice is complete and clean, without performing external registry or repository mutations.
...
- [ ] **Step 2: Inspect the branch commits and worktree state**
...
- [ ] **Step 3: Stop at the implementation boundary**

Do not create npm/PyPI organizations, publish either package, transfer the GitHub repository, edit `~/d/mindful/` or `~/d/science/`, introduce domain package layouts, or decide the Python domain import namespace. Record those as follow-up work governed by the accepted design rather than expanding this branch.
